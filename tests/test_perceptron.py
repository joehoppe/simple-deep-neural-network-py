...
from neural_network import Perceptron, WeightNode
...
def test_predicts_one_when_weighted_sum_meets_threshold():
    ...
    perceptron = Perceptron(weights, bias=0.25, threshold=1.0)
...
def test_predicts_zero_when_weighted_sum_is_below_threshold():
    perceptron = Perceptron.from_weights([0.25, -0.5], threshold=1.0)
...
def test_from_weights_accepts_injected_activation_function():
    perceptron = Perceptron.from_weights(
        [1, -1],
...
def test_rejects_input_count_mismatch():
    perceptron = Perceptron.from_weights([1, 2])
...
def test_rejects_non_numeric_weights():
    perceptron = Perceptron(WeightNode("bad"))
...
def test_rejects_non_callable_activation_function():
    with pytest.raises(TypeError):
        Perceptron.from_weights([1], activation_function=1)
